 ...
 * ===========================
 * This file converts an encoded candle byte stream into sliding-window
 * structural metrics used by the regime gate pipeline.
 *
 * Inputs:
 * - A byte stream produced by the Python manifold encoder. Each byte
 *   represents quantized candle features such as direction, delta bucket,
 *   ATR bucket, liquidity, and volume state.
 ...
 *
 * Outputs:
 * - ByteStreamWindow records for each sliding window, containing coherence,
 *   stability, entropy, rupture, a derived hazard score, and a bucketed
 *   signature used for repetition counting.
 * - ByteStreamSummary aggregate means across all analyzed windows.
 *
 * Notes on the derived fields in this file:
 * - hazard is implemented as max(0, entropy - coherence) for each window.
 * - hazard_lambda is a damping score derived from StructuralOptions rather than
 *   a market-calibrated probability estimate.
 * - signatures are rounded buckets of coherence/stability/entropy, used only
 *   for pattern repetition tracking.
 */
