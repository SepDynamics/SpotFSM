  ...
  if (!bits.empty()) {
    ...
    if (window >= 8 && bits.size() >= 2) {
      ...
      for (size_t i = start_i; i <= bits.size(); i += step) {
        ...
        /*
         * Secondary heuristic descriptors exposed in the JSON output:
         * - reynolds_ratio: |rupture| divided by |domain wall slope| when the
         *   slope is non-zero. The name is analogical; this is not a physical
         *   Reynolds number.
         * - temporal_half_life: log(2) / |coherence_tau_slope| when the slope
         *   is non-zero. This is a compact decay-timescale heuristic.
         * - spatial_corr_length: 1 / |domain wall slope| when the slope is
         *   non-zero. This is a structural length proxy, not a measured
         *   market distance.
         * - pinned_alignment: currently emitted as a constant placeholder of
         *   1.0 so downstream consumers can rely on the field existing.
         */
