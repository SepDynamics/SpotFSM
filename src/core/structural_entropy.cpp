...
bitspace::DampedValue EntropyProcessor::integrateFutureTrajectories(
  ...
  if (!local_events.empty()) {
    ...
    /*
     * Compute a normalized Shannon-style entropy over the observed structural
     * state labels in this local event window, then clamp it into the
     * operating range used by the downstream damping logic.
     */
